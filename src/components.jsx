...
export function Dropdown({ OptionsMenu, userName }) {
...
export function Dropup({ OptionsMenu, userName }) {
  var location = useLocation().pathname;
  const [openStatus, setOpenStatus] = useState(false);
  ...
  return (
    ...
    >
      ...
      <div className={`dropup_content ${openStatus ? "visible" : "hidden"}`}>
        <OptionsMenu />
      </div>
    </div>
  );
...
const LeftSideDefault = () => {
  return (
    <div style={{ display: "flex", alignItems: "center" }}>
      <img className="logo_img" src={`data:image/png;base64,${favicon}`} />
      <h1 className="main_text medium" style={{ display: "inline-block" }}>
        Provo Techspert
      </h1>
    </div>
  );
...
const RightSideDefault = ({ OptionsMenu, userName }) => {
  return <Dropdown OptionsMenu={OptionsMenu} userName={userName} />;
};
    ...
    <header className="metallic shadow_down">
      <div className="splitter">
        ...
        <div className="right push_right">
          <RightSide OptionsMenu={OptionsMenu} userName={userName} />
        </div>
      </div>
    </header>
  );
